...
Spec impacts, applied in this PR: the spec's dependency list gains `pillow` (pypdfium2 renders to raw bitmaps; `PdfBitmap.to_pil().save()` is the supported PNG-encoding path — pypdfium2 writes no image files itself) and `jsonschema` (providers must validate responses against the request's JSON Schema; pydantic validates pydantic models, not arbitrary JSON Schema documents); the report example and § Extraction report gain the `schema_version`/`osrforge_version` fields work item 4 pins, so the spec stays the authority on the report shape; and the overrides v1 field list gains `name` (osrlib's `AreaSpec.name` is real and extraction gets room names wrong often enough that correcting one shouldn't require replacing the whole area).
...
- `tests/assets/README.md` documents every asset's provenance and license. Two assets:
    ...
    - **`tests/assets/<module>/`** — the spike module: a short (≤ 32 pages, single dungeon) Basic Fantasy RPG adventure. BFRPG relicensed to CC BY-SA 4.0 with its 4th edition (June 2023); candidate: JN1 The Chaotic Caves. **Verification is a spike step, not an assumption:** confirm the chosen module PDF's own license page states CC BY-SA 4.0 and its contributors appear on the project's consent list before committing anything. The directory carries the CC BY-SA 4.0 notice, attribution, source URL, and the PDF's sha256, and everything derived from the module — page renders *and* recorded fixtures, whose request digests embed module text — lives inside this fenced directory. Budget for the whole directory: 10 MiB (the PDF, the page subset backing replay-grade fixtures, and the fixtures themselves; work item 10 defines the replay-grade/evidence-grade split). This mirrors osrlib's split: MIT code, separately-licensed content, clearly fenced. It is also distinct from the eval corpus: phase 4's corpus ships pointers + hashes and never bundles PDFs, per the spec — this one committed, license-verified module is a test asset, not the corpus.
...
- `contracts/run.py`:
    ...
    - `Stage` — a `StrEnum` pinning the spec's stage names as wire values: `preprocess`, `survey`, `content`, `monsters`, `geometry`, `assemble`. These key `run.json`'s stage table; changing one later is a schema-version event. (They do *not* name the `stages/` cache files — the spec's workdir layout pins those as `survey.json`, `areas.<dungeon>.<level>.json`, and `monsters.json`, and only the model-calling stages have caches.)
    - Spec inconsistency surfaced for phase 2, with a pinned default reading: the pipeline section lists geometry as stage 4, but § CLI's `convert` chain (`preprocess → survey → content → monsters → assemble`) omits it, and the workdir layout has no geometry cache file. The reading that reconciles all three: geometry is deterministic, so it is recomputed inside every assembly rather than cached — which is also what keeps assembly pure while geometry stays correctable through overrides. `run.json` still tracks all six stages so status and flag attribution have a home; its `geometry` entry completes with assembly. Phase 2 confirms this reading when it implements both stages, or drives a spec fix.
...
- Request fingerprinting, shared by fixtures and future run-caching: canonical JSON (sorted keys, compact separators, UTF-8) of the request with each image part replaced by `{"sha256": ..., "bytes": ...}`; the fingerprint is the sha256 hex of that canonical form, exposed as `ModelRequest.fingerprint()`. `tag` participates in the fingerprint (it's part of request identity; tags are stable identifiers — pipeline stage names or spike probe ids — not free prose).
...
- Structured output: native JSON-schema response format when the spike confirms gpt-5.4 supports it; otherwise validate-and-retry (parse, `jsonschema`-validate, and on failure re-prompt with the validation errors appended — at most two re-prompts, three attempts total, then `SchemaValidationError`). Either way the provider owns enforcement — the pipeline never sees invalid `data`.
...
- `preprocess(pdf_path, workdir_path, settings) -> RunMeta`, deterministic code only:
    ...
    4. Per page, 1-based, zero-padded to 4 digits (`0001.png` per the spec's layout; `max_pages` keeps 4 digits sufficient): render at `scale = render_dpi / 72` → `bitmap.to_pil().save(...)` as RGB PNG; extract the text layer via `get_textpage().get_text_bounded()` (the fully-Unicode API; pypdfium2 documents `get_text_range()` as UCS-2-limited, and module text can carry non-BMP glyphs), normalize newlines to `\n`, write `NNNN.txt` UTF-8 — an empty or missing text layer yields an empty file (the spec's scanned-module behavior), never a skipped one.
...
- **Module selection and licensing gate** (work item 2's verification step) precedes any recording. Commit the PDF, the page assets replay-grade fixtures reference, the attribution README, and hashes — all inside `tests/assets/<module>/`, fixtures under `tests/assets/<module>/fixtures/`.
- **Two grades of recording**, so replayability doesn't force committing every page of the module: *replay-grade* fixtures (the structured-output probes and the extraction smokes, each built over a bounded page subset — at most 8 pages) are committed together with their exact page PNGs and must replay through `FixtureProvider` in tests; *evidence-grade* recordings (the boundary probes: image-count ceiling, whole-module context) back claims in the findings document and are committed as fixture files without their page assets — replay is not promised.
...
- **versioning/errors**: `SCHEMA_VERSION` importable and equal to 1; `osrforge_version()` matches installed package metadata; every typed exception derives from `OsrForgeError`.
...
- Real fixtures from the license-verified spike module are committed with attribution inside the fenced asset directory — replay-grade ones alongside the exact page assets they were recorded against, and within work item 2's size budget.
...
- **The live spike ran on 2026-07-09** (initially deferred for missing credentials; Azure access arrived via the `az` CLI before the PR merged, so the deferral lasted a day). Findings: `docs/foundry-capabilities.md`; fixtures and pages: `tests/assets/chaotic-caves/`. Two of work item 2's assumptions did not survive the licensing gate, exactly the outcome the gate exists to catch:
    - **The spike-module PDF is not committed.** BFRPG modules license their text, maps, floorplans, diagrams, charts, and forms under CC BY-SA 4.0, but the license page carves out most artwork ("used with permission … you must remove all non-licensed artwork before doing so") — and the non-consenting credited artists are indeed absent from the project's consent list. The as-published PDF therefore cannot be redistributed. The fenced directory instead carries the PDF's sha256 + source pointer (the PDF itself is gitignored), and the committed replay-grade page subset uses only pages visually verified to contain nothing but licensed text and maps. Work item 2's 10 MiB directory budget now covers pages + fixtures only (~4.3 MiB actual).
    - **The ≤ 32-page criterion does not exist in the current BFRPG catalog** — JN1 The Chaotic Caves r28, at 48 pages, is the *shortest* adventure available. JN1 stays the module; the whole-module context probe absorbed the extra pages without trouble.
- **Fixture request digests gained the request's JSON Schema** (additive, same artifact schema version, all spike fixtures recorded in the new form). Work item 6 pinned the digest as system + text parts + image sha/size; without the schema, a fixture was not reconstructable into its request, so replay-grade fixtures could not actually replay in tests without duplicating the spike's prompt code. `tests/test_spike_fixtures.py` now rebuilds every replay-grade request from its fixture plus the committed pages and replays it through `FixtureProvider`.
- **Dev-group additions beyond work item 1's pinned list** (`pytest`, `ruff`, `pyright`), each load-bearing for the pinned pyright-strict-on-`src/` decision: `types-pyyaml` and `types-jsonschema` (neither pyyaml nor jsonschema ships a `py.typed` marker, so strict mode needs their typeshed stubs), `azure-identity` (dev-only, so the Entra client-build path is unit-testable and its import resolves under pyright), and a local `typings/pypdfium2/` stub covering exactly the pypdfium2 surface preprocessing uses (no upstream stubs exist).
