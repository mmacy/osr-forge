...
- **The live spike is deferred to a manual run, not delivered by the implementation PR.** Work item 10's live steps — the module selection and licensing gate, the recorded probes, and `docs/foundry-capabilities.md` — need Azure Foundry credentials, which the implementation environment does not have. The probe scripts (`tools/spike/`) are implemented and reviewed; the three Definition-of-done bullets that depend on live recording (the findings document, the committed spike-module asset, and its real fixtures) are deferred to that run. The spike must happen before phase 1 *planning*, since phase 1's chunking and schema decisions rest on its findings.
