...
- **No pixel-perfect map reproduction.** Geometry is synthesized from the extracted room-connection graph — informed by the printed map where the improvement arc's map-reading stage proposes structure (phases 11–12, `docs/improvement-spec.md`) — and corrected by humans; reproducing the printed cartography exactly stays out of scope, and the human remains the last word on geometry.
...
Seven stages. Stages 1–4 call the model; 0, 5, and 6 are deterministic code.
...
4. **Monsters (3).** Resolve extracted names against `osrlib.data.load_monsters()` in tiers: normalized exact id match → curated alias table → stdlib fuzzy match (`difflib`) → one LLM pass that picks from the top-k candidates or answers "none of these" (and prefers null on doubt — an unmatched name keeps its own printed stat block downstream, while a wrong pick silently replaces the module's creature). The stat-block pass then runs over the names still unresolved plus the LLM- and fuzzy-resolved ones, gated by the `custom_monsters` knob: one transcription request per name over its planned page set — the union of its encounters' source pages and every page whose text layer contains the name, capped, text plus images (images because tabular stat blocks scramble in extracted text) — cached raw in `stages/statblocks.json` with an explicit absent marker for a name whose pages print nothing. The pass transcribes the printed block system-neutrally; every rules judgment lives in assembly's deterministic mapping. After the pass, a deterministic veto judges each LLM and fuzzy pick against its printed block: a usable block whose Hit Dice count differs from the picked template's by one or more flips the pick to unresolved in the cache — the vetoed name flows into the custom-emission path, and the discarded pick rides the cache and the report's monsters summary (AC never vetoes; near-misses are assembly's `resolution_suspect` flag). Names with no usable block become report flags, and the draft never carries a dangling id — that would fail `validate_adventure`, and osrlib refuses to open a session over an invalid adventure. What the draft carries for that residue is the `unresolved_fallback` setting's call: `best-effort` (the default) substitutes a deterministic, level-appropriate stand-in from osrlib's shipped dungeon encounter tables, flagged with both names; `omit` leaves the encounter out. Either way, a human override choosing the true substitute is the last word.
5. **Map reading (4).** One structured-output request per surveyed level over its unblanked printed map pages, planned off the survey cache alone: the model proposes same-level adjacency pairs between the level's printed keys, doors on those pairs (`door`/`secret_door` per the map's legend — a proposed pair's door reading is complete, so `none` is an assertion), and the map-labeled entrance — and nothing more. No content-stage output rides the request: the map read is an independent second opinion on the prose, and feeding it prose connections would let one instrument contaminate the other. Proposals cache as answered, raw, in `stages/mapread.json`; endpoint resolution happens at reconcile time in deterministic code, so a reconciliation-policy fix never strands a recorded fixture. Degenerate readings record themselves honestly instead of asserting: a level with no map pages, all pages blanked, or no keyed areas makes no request, and a zero-adjacency answer over a multi-area level records `unread:empty_reading` rather than disputing every prose edge. The `map_reading` knob (`read`/`off`) is the stage's cost control; `off` writes the knob-echoed cache with zero levels — the prose-only pipeline, bit for bit.
6. **Geometry (5).** Deterministic synthesis of grid geometry from the room graph, the map reading reconciled in, recomputed inside every assembly rather than cached (which is why the workdir has no geometry cache file). Reconciliation first (`osrforge.reconcile`, the shared policy the eval scorer consumes too): each level's prose edge facts merge with its map proposals under the precedence rule — a stated prose fact survives any map conflict, flagged; a prose absence fills from the map, flagged as adopted; agreement is silent — so map-only pairs join the placement graph, adopted doors realize like prose doors (conditions unstated — the map cannot state them), and every adoption, disagreement, and dropped proposal lands as a `map_disputed` flag. The entrance selection is shared the same way: a resolvable map-proposed entrance beats the positional heuristic (first listed area of the lowest-numbered non-empty level), flagged when the two differ. Then placement: anchor the entrance level on the selected entrance area, walk the connection graph breadth-first, place rooms as rectangular cell clusters (10' per cell, sized from stated dimensions like "30' × 40'" when present, defaults otherwise) in the stated compass direction when known, route 1-cell-wide corridors, and shift on collision. After placement, every resolved edge the BFS tree left unrealized — cycle-closing and re-anchored alike — routes a corridor over free and existing-corridor cells, so a printed loop plays as a loop and a stated door has a wall to land on; emit open `Edge`s — a connection whose stated mechanism is a door or secret door realizes as a `door` edge on the stating room's wall, a genuinely walled-in pair drops the door fact with a `connection_ambiguous` flag, and a doorless walled-in pair flags `connection_ambiguous:edge to <key> not routed` — an unrealized connection is never silent. Vertical links synthesize transitions: keyed targets as stated, level-shaped targets under a total landing policy (opposite-sense links pair into one stairway, leftovers land on the target level's first keyed area, every guessed landing flagged `transition_guessed`); trapdoors and chutes are one-way, stairs reciprocal; transitions stay prose-derived — per-level map requests cannot express a cross-level pair. Overrides remain the last word on all of it. The output always satisfies osrlib's structural rules — entrance exists, transitions align, cells in bounds — even when it doesn't match the printed map. Every synthesized area is flagged `geometry_synthesized` in the report.
7. **Assemble and check (6).** Apply `overrides.yaml` to the cached stage outputs, map cached stat blocks into custom `MonsterTemplate`s (deterministically, under a pinned per-format policy: a block is usable when it carries an AC plus an HD line or a class-level notation — the refusal ladder — and every derived, defaulted, or discarded-and-rederived field is flagged `monster_custom` and recorded in the report), bundle the templates built encounters actually reference into `Adventure.monsters`, build the `Adventure` via `model_validate`, run `validate_adventure` against the shipped monster/equipment catalogs (which unions bundled templates internally), write the four artifacts. Validation failures land in the report rather than crashing — a draft is allowed to be invalid; *publishing* it somewhere is the consumer's gate.
...
Flag vocabulary is small and enumerated (geometry synthesized, monster unresolved, monster custom, low confidence, connection ambiguous, transition guessed, treasure unparsed, page unreadable, resolution suspect, survey disputed, map disputed) so UIs can badge reliably. `low_confidence:<value> self-assessed` marks an area whose cached confidence sits below the pinned 0.6 floor — the model's own uncertainty made visible, explicitly not a hallucination guard; `resolution_suspect` marks a surviving non-exact monster pick whose printed block disagrees on a non-vetoing axis; `survey_disputed` carries each census disagreement at module scope; `map_disputed` carries every map/prose adoption and disagreement (edge disputes on the survey-order-first endpoint, the entrance dispute on the selected entrance area) and every dropped proposal (module scope, the level address in the detail) — the detail always names both readings. `monster_custom` is the review badge for an emitted template; the monsters summary's `custom` records are the review detail — per bundled template, its id, source name, transcription pages, and every field the mapping derived rather than read off the printed page.
...
- **Evals** are a separate, on-demand harness (not per-commit CI): a corpus of freely licensed adventures (`tools/eval/corpus/`) with verified structural ground truth, authored from the printed module under the independence discipline (`tools/eval/AUTHORING.md`) — never from pipeline output. The scorer (`osrforge.evals`) reads the stage caches and reports seven metric families: areas (recall + precision, plus dungeon alignment counts), encounters (name recall, count accuracy, resolution accuracy, custom-emission accuracy — a truth encounter with `template` asserts the SRD resolution; one with `custom: true` instead asserts *this creature should emit* and matches when the stat-block cache carries a usable block under assembly's own refusal-ladder predicate; omitted-without stays `non_srd` — and name precision over areas whose truth asserts encounters, riding the asserted-empty convention: `encounters: []` asserts a verified-empty area, an omitted key asserts nothing), connections (F1 over undirected same-level edges within the truth's asserted universe), treasure (presence agreement and letter accuracy, within the truth's asserted universe), doors (presence recall and precision plus kind and locked accuracy over the asserted door universe, where an asserting area's door set is complete), transitions (dungeon-scoped vertical links matched undirected on truth endpoint pairs, with kind accuracy), and entrance (does the selection pick the printed way in, per asserting dungeon). The edge families' fact source is the *reconciled* set — the prose facts merged with the workdir's map reading through the same shared policy geometry consumes, and the entrance family calls the same shared selection — so the metrics measure exactly what the pipeline builds, and a workdir with no map cache scores prose-only, never an error. Results live on the committed scoreboard, `tools/eval/corpus/scoreboard.json`. Extraction is nondeterministic run to run, so the baseline sweep runs twice and the regression band is the observed per-metric spread floored at 0.02 absolute; any PR that edits extraction prompts or schemas, the alias table, resolution logic, or the model deployment re-runs the sweep and commits the updated scoreboard in the same PR, a PR that changes the scorer's matching or metric semantics re-scores the standing sweep pair offline and refreshes the band the same way, and a metric dropping by more than the band requires explicit justification in the PR description. The corpus ships pointers + hashes, not PDFs. Private (BYOM) corpora over locally owned, non-redistributable modules side-load through the same harness and scorer (`--corpus DIR`, identical layout, integrity via a local hash sidecar when the manifest cannot pin one). The committed BYOM scoreboard (`tools/eval/byom-scoreboard.json`, fed by an explicit `publish` step) is advisory — owner-refreshed, `osrforge_version`-stamped — beside the gating corpus scoreboard.
...
9. **Phase 8 — documentation retargeting.** The published docs and docstrings rewritten for the outside reader: a glossary of the project's terms of art, published contributor pages (setup and gates, architecture, the testing model), docstrings retargeted off the unpublished spec and phase plans onto published anchors, Examples on the first-touch API surfaces, and per-field documentation on the contract and metric models. Milestone: a new contributor goes from the published site to a passing local development loop without reading any unpublished document. (Owner-approved deviation: no plan PR; the implement-side rubber-duck loop still runs.)
10. **Phase 9 — measurement extension.** The improvement arc's yardstick (the arc — phases 9–12 plus companion workstreams — is specified in `docs/improvement-spec.md`): door, transition, and entrance metric families, an encounter-precision metric with the asserted-empty truth convention, the in-phase truth passes over every corpus member, an offline re-score of the standing sweep pair with refreshed bands, and a license-verified non-BFRPG committed member (with its own baseline sweep) if a candidate passes the phase 0 procedure. Milestone: structure and hallucination quality are measured numbers with bands before any extraction change ships.
11. **Phase 10 — resolution and survey hardening.** Stat-aware monster resolution (the printed stat block becomes a deterministic veto on non-exact picks), a survey self-consistency census, and confidence thresholding — new flags for all of it. Milestone: a wrong resolution can no longer be silent, and a mode-flipped survey flags itself in the run it happens in.
12. **Phase 11 — map-grounded structure.** A map-reading extraction stage proposing per-level adjacency, doors, and the entrance, deterministically reconciled with prose connections in geometry synthesis; every disagreement flagged. Milestone: the phase 9 structure metrics re-measured with the map in play — the arc's bet that edge precision moves.
13. **Phase 12 — map-grounded shape.** Map-proposed room footprints replacing the dimension-regex rectangles, contingent on phase 11's measured result (the go/no-go reading is recorded in `docs/improvement-spec.md` before this phase is planned). Milestone: the phase 9 structure metrics hold or rise with footprints in play, judged with a structural-counts comparison against printed dimensions — the aim being that geometry overrides become the exception rather than the routine.

## Open questions

- **gpt-5.4 capabilities** (schema outputs, image inputs, context, cost) — phase 0 spike answers this and may reshape chunking.
- **Vision-first geometry.** A model pass that proposes *map-image* geometry — cell clusters and corridors matching the printed cartography — with the human still confirming. Narrowed again by phase 11: the map now proposes adjacency, doors, and the entrance through the mapread stage, so the open question is footprints alone — phase 12, contingent on phase 11's measured result (`docs/improvement-spec.md`).
