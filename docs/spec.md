...
10. **Phase 9 — measurement extension.** The improvement arc's yardstick (the arc — phases 9–12 plus companion workstreams — is specified in `docs/improvement-spec.md`): door and transition metric families, an encounter-precision metric with the asserted-empty truth convention, the in-phase truth passes over every corpus member, an offline re-score of the standing sweep pair with refreshed bands, and a license-verified non-BFRPG committed member if a candidate passes the phase 0 procedure. Milestone: structure and hallucination quality are measured numbers with bands before any extraction change ships.
...
13. **Phase 12 — map-grounded shape.** Map-proposed room footprints replacing the dimension-regex rectangles, contingent on phase 11's measured result (the go/no-go is recorded in `docs/improvement-spec.md` before this phase is planned). Milestone: geometry overrides become the exception rather than the routine on the committed corpus.
