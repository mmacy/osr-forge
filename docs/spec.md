...
- **Vision-first geometry.** A model pass that proposes *map-image* geometry — cell clusters and corridors matching the printed cartography — with the human still confirming. Narrowed by phase 6: prose-signal doors and stairs are extracted now, so the open question is the map image alone. Scheduled: the improvement arc takes it in two measured steps — adjacency and doors in phase 11, footprints in phase 12, contingent on 11's result (`docs/improvement-spec.md`).
